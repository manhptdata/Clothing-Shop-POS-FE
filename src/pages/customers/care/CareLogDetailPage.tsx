...
import { useGetCareLogByIdQuery } from "@/redux/api/customerApi";
...
export default function CareLogDetailPage() {
  ...
  // Gọi API lấy chi tiết care log
  const { data: responseData, isLoading } = useGetCareLogByIdQuery(id || "");

  const log = responseData?.data;
  ...
  if (!log) {
    return (
      <div className="p-6 text-center text-gray-500">
        <p>Không tìm thấy lịch sử chăm sóc này.</p>
        <Button
          variant="outline"
          onClick={() => navigate(-1)}
          className="mt-4"
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto w-full text-gray-800 antialiased">
      {/* Header */}
      <header className="mb-6 flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-2">
            ...
            <Button
              size="sm"
              variant="primary"
              className="flex-1 whitespace-nowrap !bg-amber-500 hover:!bg-amber-600 !border-amber-500"
              leftIcon={<i className="fa-solid fa-pen-to-square"></i>}
              onClick={() => navigate(`/customers/care/history/log/${id}/edit`)}
            >
              Sửa nhật ký
            </Button>
          </div>
