...
import { useUpdateReceiptMutation, useGetReceiptByIdQuery } from '@/redux/api/receiptApi';
...
export default function ReceiptUpdatePage() {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();

    const { data: receipt, isLoading: isFetching } = useGetReceiptByIdQuery(Number(id), { skip: !id });
    ...
    useEffect(() => {
        if (receipt) {
            ...
            const initialLines: ReceiptLine[] = (receipt.items ?? []).map(item => ({
                key: `${item.variantId}-${Date.now()}-${Math.random()}`,
                variantId: item.variantId || 0,
                productName: 'Sản phẩm ' + item.variantId,
                sku: item.sku || '',
                variantLabel: '',
    ...
    return (
        <div className="max-w-[1200px] mx-auto w-full">
            <div className="flex items-center gap-3 mb-6">
                <button
                    onClick={() => navigate(`/warehouse/receipts/${id}`)}
                ...
                <div>
                    ...
                    <p className="text-sm text-on-surface-variant">Cập nhật thông tin phiếu {receipt?.code}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2">
                    ...
                    <div className="flex justify-end gap-3 mt-4">
                        <Button variant="outline" onClick={() => navigate(`/warehouse/receipts/${id}`)}>
                            Hủy
                        </Button>
                        <Button
